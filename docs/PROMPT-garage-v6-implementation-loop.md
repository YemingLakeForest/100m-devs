...
The v6 image is the spatial and visual target for:
...
- Restore the compact leadership room in the far-left/back zone shown by v6.
...
- Use the v6 floor's dark warm-brown concrete value, not a pale lilac slab.
...
- Maintain a strict one-light-to-three-planter ratio on each visible street side
  independently. Match v6's authored counts: the gate/frontage side has two
...
1. **Inspect the target and current frame.**
   Open v6 at original resolution. Capture the current 20-developer scene at
...
3. Produce a final side-by-side comparison with v6.
...
- the live 20-developer screenshot is immediately recognizable as v6 without
