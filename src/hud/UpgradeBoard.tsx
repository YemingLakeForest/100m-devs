...
function NodeView({
  ...
  const style = { left: (node.x + 1.5) * CELL - NODE / 2, top: (node.y + 1.5) * CELL - NODE / 2 }
  ...
  return (
    <Panel open={open} from="right" className="hud__upgrades">
      <div className="upgrade-board">
        ...
        <div className="upgrade-board__scroll">
          <div className="upgrade-board__world" style={{ width: worldW, height: worldH }}>
            ...
            {TECH_TREE.map((node) => (
              <NodeView key={node.id} node={node} state={state} shifts={shifts} onOpen={setSelected} />
            ))}
          </div>
        </div>
